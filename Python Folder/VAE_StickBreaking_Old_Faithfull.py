...
def calculate_loss(reconstructed1,target, alpha,beta,alpha0,beta0):
   ...
   if __name__ == '__main__':
    ...
    stan_data = {'K':input_dim,
                ...
                'cov1': np.identity(input_dim),
                'mu1': np.array([0]*input_dim),
                'cov2': np.identity(input_dim)*0.25,
                'mu2': np.array([0]*input_dim),
                'ones':np.ones(input_dim)}
        
    #### stan code
    sm = pystan.StanModel(file='VAE_SB_Stan_old_faithful_beta.stan')
    fit = sm.sampling(data=stan_data, iter=10000, warmup=500, chains=3)
    out = fit.extract(permuted=True)
    print(fit)
    
    df1 = pd.DataFrame(out['pi'])
    df2 = pd.DataFrame(out['theta_mu_star'])
    df3 = pd.DataFrame(out['theta_sd_star'])

    x_new = np.linspace(-4, 4, 200, endpoint=False)
    post_pdf_contribs = scipy.stats.norm.pdf(np.atleast_3d(x_new),
                                      out['theta_mu_star'][:, np.newaxis, :],out['theta_sd_star'][:, np.newaxis, :]
                                      )
    ...
    plt.xlim(-4,4)
    plt.hist(old_faithful_df['std_waiting'], bins=20, normed=True, alpha=0.5,histtype='stepfilled', color='steelblue',edgecolor='none')
    ...
    plt.savefig('Inference Fit for Old Faithful (Beta).png')
    ...
    plt.tight_layout()
    plt.savefig("VAE Traceplot Old Faithful (Beta).png")
    ...
    data3=fit.summary('theta_sd_star')["summary"][:, a].reshape(1,input_dim)
    ...
    for i in range (input_dim):
        name.append('sigma'+str(i+1))
    for i in range (input_dim):
        name.append('pi'+str(i+1))
    np.array(name).reshape(1,z_dim+3*input_dim)
    ...
    plt.savefig("VAE Effective Sample Size Old Faithful (Beta).png")
    ...
    data_3=fit.summary('theta_sd_star')["summary"][:, a_].reshape(1,input_dim)
    data_4=fit.summary('pi')["summary"][:, a_].reshape(1,input_dim)
    ...
    df_=pd.concat([df_1, df_2.reindex(df_1.index),df_3.reindex(df_1.index),df_4.reindex(df_1.index)], axis=1)
    ...
    ax.xaxis.set_tick_params(labelsize=15)
    ax.xaxis.set_ticks(np.arange(-1,10,10))
    ax.set_xlabel('$\hat{R}$', fontsize = 15)
    ax.set_title('$\hat{R}$ of MCMC', fontsize = 20)
    plt.savefig("VAE Rhat Old Faithful (Beta).png")
