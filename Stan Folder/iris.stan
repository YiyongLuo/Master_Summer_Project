data {
    ...
    int<lower=0> n1; // hidden layer dim
    int<lower=0> m1; // latent dim
    int<lower=0> N; // number of observation
    matrix[4,N] x; //known observations
    matrix[m1, n1] W1; // weight for latent layer to hidden layer in decoder
    matrix[1, n1] b1; // bias for latent layer to hidden layer in decoder
    matrix[n1, K] W3; // weight for hidden layer to output layer in decoder
